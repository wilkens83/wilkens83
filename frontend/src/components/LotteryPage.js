...
const LotteryPage = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Closed': return 'text-red-400 bg-red-900/20';
      case 'In Progress': return 'text-yellow-400 bg-yellow-900/20';
      default: return 'text-gray-400 bg-gray-900/20';
    }
  };

  const renderDrawCarousel = () => {
    return (
      <div className="mb-6">
        <label className="block text-sm text-gray-300 mb-3">Chwazi Tiraj / Select Draw</label>
        <div className="flex gap-3 overflow-x-auto pb-2">
          {drawsSchedule.map(draw => (
            <div
              key={draw.id}
              onClick={() => setSelectedDraw(draw.id)}
              className={`min-w-[140px] p-3 rounded-xl border-2 cursor-pointer transition-all ${
                selectedDraw === draw.id
                  ? 'border-blue-400 bg-blue-600/30'
                  : 'border-slate-600 bg-slate-700/50 hover:border-slate-500'
              }`}
              data-testid={`draw-${draw.id}`}
            >
              <div className="text-sm font-semibold text-white mb-1">
                {draw.name}
              </div>
              <div className="text-xs text-gray-300 mb-2">
                {draw.time} {draw.timezone}
              </div>
              <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(draw.status)}`}>
                {draw.status}
              </span>
            </div>
          ))}
  ...
  const renderGameTypeButtons = () => {
    return (
      <div className="mb-6">
        <label className="block text-sm text-gray-300 mb-3">Game Types</label>
            ...
            <button
              ...
              className={`p-4 rounded-xl border-2 text-left transition-all transform hover:scale-[1.02] ${
                selectedGameType === gameType.id
                  ? 'border-white bg-gradient-to-r from-blue-600 to-blue-700 shadow-lg'
                  : `border-slate-600 ${gameType.color} hover:border-slate-500 shadow-md`
              }`}
